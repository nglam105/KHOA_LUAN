    ...
    def test(self, test_mode=1):
        ...
        for i, data_batch in enumerate(self.test_loader):
            ...
            if test_mode == 0:
                ...
                cv2.imwrite(os.path.join(self.config.test_fold, name[:-4]+ + '_' + mode_name[test_mode] + '.jpg'), multi_fuse)
            elif test_mode == 1:
                with torch.no_grad():
                    ...
                    multi_fuse = 255 * pred
                    cv2.imwrite(os.path.join(self.config.test_fold, self.config.output), multi_fuse)
        time_e = time.time()
