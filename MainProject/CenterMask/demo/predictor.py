...
class COCODemo(object):
    ...
    def run_on_opencv_image(self, image,args):
        ...
        # print (predictions.get_field("mask").size())
        top_predictions = self.select_top_predictions(predictions)
        # print(top_predictions.get_field("mask").size())
        # mask_result=np.zeros(shape=(image.shape[0],image.shape[1],1),dtype=np.uint8)
        # print(mask_result.shape)
        salientMap = cv2.imread(args.salient, 0)
        ...
        if args.selection==1:
            for i in range(top_predictions.get_field("mask").size()[0]):
                #if top_predictions.get_field("scores")[i]<0.4:
                temp=top_predictions.get_field("mask")[i][0].numpy() *255
                temp=np.expand_dims(temp,2)
                #cv2.imshow("mask",np.expand_dims(temp,2))
                #cv2.imwrite(f"output/mask_{i}.png",temp)
                # mask_result=cv2.bitwise_or(mask_result,temp)
                ret, temp = cv2.threshold(temp, 96, 255, cv2.THRESH_BINARY)
                Intersect = cv2.bitwise_and(temp, salientMap)
                num_intersect = np.count_nonzero(Intersect)
                num_temp = np.count_nonzero(temp)
                print(f"{top_predictions.get_field('scores')[i]} {num_intersect} {num_temp}")
                if num_intersect / num_temp < 0.75:
                    maskNotSalient = cv2.bitwise_or(maskNotSalient, temp)
        ...
        cv2.imwrite(args.map_dir,maskNotSalient)
        result = image.copy()
    ...
    def computeIOU(self,mask1,mask2):
        intersect=np.zeros(shape=(image.shape[0],image.shape[1],1),dtype=np.uint8)
        union=np.zeros(shape=(image.shape[0],image.shape[1],1),dtype=np.uint8)
