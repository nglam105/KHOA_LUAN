...
if __name__ == "__main__":
    ...
    print(dirs[0])
    training_folder = os.listdir(args.folder_path+"/"+dirs[0])
    for training_item in training_folder:
        if "_edge" not in training_item:
            training_item = args.folder_path+"/"+dirs[0]+"/" + training_item
            training_file_names.append(training_item)

    validation_folder = os.listdir(args.folder_path+"/"+dirs[1])
    for validation_item in validation_folder:
        if "_edge" not in validation_item:
            validation_item = args.folder_path+"/"+dirs[1]+"/" + validation_item
